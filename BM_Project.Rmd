...
BM_data_filter <- subset(BM_data_filter, subset =  nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 15)
...
results2 = setdiff(tmp2$gene,tmp1$external_gene_name) # Not all of the gene names might be found, so the description list can be shorter than the original

results2

genedesc[nrow(genedesc) + 1,] = list(results2,vector("list", length(results2)))
...
genedesc
top10$gene
index1 <- match(genedesc$external_gene_name,top10$gene)
genedesc$index = index1


genedesc['order'] = 
vector1 = genedesc$external_gene_name
#result = genedesc[order(index1)] 
#result = genedesc['description'][final] 

result = genedesc[
  with(genedesc, order(index1)),
]
